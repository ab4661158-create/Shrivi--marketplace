...
const scriptGroups={
  ...
  seller:[
    ...
    ['seller-8-slots.js','seller-8-slots.js']
  ]
};

const allScripts=[...scriptGroups.common,...scriptGroups.customer,...scriptGroups.seller];
const assets=Object.fromEntries(allScripts.map(([url,file])=>['/'+url,fs.readFileSync(path.join(__dirname,file),'utf8')]));
const makeTags=(groups)=>groups.flatMap(group=>scriptGroups[group]).map(([url])=>`<script src="/${url}?v=32"></script>`).join('\n');
...
const server=http.createServer((req,res)=>{
  ...
  const proxyReq=http.request({hostname:'127.0.0.1',port:INTERNAL_PORT,method:req.method,path:req.url,headers:{...req.headers,host:`127.0.0.1:${INTERNAL_PORT}`,'accept-encoding':'identity'}},proxyRes=>{
    ...
    const isHtml=req.method==='GET'&&/text\/html/.test(type);
    const groupMap={
      '/shop':['common','customer'],
      '/shop/':['common','customer'],
      '/seller':['seller'],
      '/seller/':['seller'],
      '/':['common'],
    ...
    const groups=groupMap[pathname];
    const inject=isHtml&&groups;
    ...
    const chunks=[];proxyRes.on('data',c=>chunks.push(c));proxyRes.on('end',()=>{
      ...
      html=html.replace(/<script[^>]+(?:shrivi-features|shrivi-upgrade-suite|amazon-style-upgrade|shrivi-production-upgrade|shrivi-customer-backend|shrivi-app-init|seller-pro-listing-ui|seller-gallery-v2|seller-multi-images|seller-multi-select-fix|seller-8-slots|customer-gallery-v2|seller-final-fix|seller-images-router-fix)[^>]*><\/script>/gi,'');
      const tags=makeTags(groups);
      html=/<\/body>/i.test(html)?html.replace(/<\/body>/i,`${tags}\n</body>`):`${html}\n${tags}`;
      const headers={...proxyRes.headers};
      delete headers['content-length'];delete headers.etag;delete headers['content-encoding'];
  ...
  proxyReq.on('error',()=>{if(!res.headersSent)res.writeHead(502,{'content-type':'text/plain; charset=utf-8'});if(!res.writableEnded)res.end('Shrivi service temporarily unavailable.')});
  req.pipe(proxyReq);
});
