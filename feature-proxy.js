...
const tags=scripts.map(([url])=>`<script src="/${url}?v=7"></script>`).join('\n');

// Preload the PostgreSQL-backed upgrade routes into the real Express server.
const child=spawn(process.execPath,['-r',path.join(__dirname,'shrivi-db-upgrades.js'),'-r',path.join(__dirname,'seller-images-upgrade.js'),'-r',path.join(__dirname,'seller-images-router-fix.js'),path.join(__dirname,'server.js')],{env:{...process.env,PORT:String(INTERNAL_PORT)},stdio:'inherit'});
...
const server=http.createServer((req,res)=>{
  ...
  const proxyReq=http.request({hostname:'127.0.0.1',port:INTERNAL_PORT,method:req.method,path:req.url,headers:{...req.headers,host:`127.0.0.1:${INTERNAL_PORT}`,'accept-encoding':'identity'}},proxyRes=>{
    ...
    const chunks=[];proxyRes.on('data',c=>chunks.push(c));proxyRes.on('end',()=>{
      ...
      html=html.replace(/<script[^>]+(?:shrivi-features|shrivi-upgrade-suite|amazon-style-upgrade|shrivi-production-upgrade|shrivi-customer-backend|shrivi-app-init|seller-multi-images)[^>]*><\/script>/gi,'');
