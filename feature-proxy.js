...
// SINGLE Seller Gallery backend:
// seller-images-upgrade.js owns the seller image/product routes.
// Do not preload the old v2/runtime/router patches here; they register the
// same endpoints again and can make Express hit the wrong handler.
const scripts=[
...
const tags=scripts.map(([url])=>`<script src="/${url}?v=28"></script>`).join('\n');

const child=spawn(process.execPath,[
  '-r',path.join(__dirname,'shrivi-db-upgrades.js'),
  '-r',path.join(__dirname,'image-upload-bootstrap.js'),
  '-r',path.join(__dirname,'seller-images-upgrade.js'),
