...
const server = http.createServer((req, res) => {
  ...
  if (req.url === '/shrivi-upgrade-suite.js') {
    res.writeHead(200, {'content-type':'application/javascript; charset=utf-8','cache-control':'no-store'});
    return res.end(upgradeScript);
  }
  ...
  const proxyReq = http.request({
    ...
    const inject = isHtml && (req.url === '/shop' || req.url === '/shop/' || req.url === '/seller' || req.url === '/seller/' || req.url === '/' || req.url === '/admin' || req.url === '/admin/');
    ...
    proxyRes.on('end', () => {
      ...
      if (!html.includes('/shrivi-upgrade-suite.js')) html = html.replace(/<\/body>/i, `${tags}</body>`);
