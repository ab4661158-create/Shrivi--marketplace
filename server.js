...
app.post(
  ...
  async (req, res) => {
    try {
      ...
      try {
        sellerId = await validateSellerId(
          req.body?.seller_id
        );
      } catch (e) {
        if (e.message === "INVALID_SELLER") {
          return res.status(400).json({
            error: "Invalid seller ID"
          });
        }

        if (e.message === "SELLER_NOT_FOUND") {
          return res.status(400).json({
...
app.get(
  ...
  async (req, res) => {
    try {
      ...
      res.json({
        products:
          integer(products.rows[0].count),

        orders:
          integer(orders.rows[0].count),

        sellers:
          integer(sellers.rows[0].count),

        customers:
          integer(customers.rows[0].count)
      });
    } catch (error) {
