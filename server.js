...
async function requireCustomer(req, res, next) {
  try {
    ...
    const id = integer(req.session.customer.id);
...
async function requireSeller(req, res, next) {
  try {
    ...
    const id = integer(req.session.seller.id);
...
app.post("/api/seller/login", async (req, res) => {
  try {
    ...
    const result = await pool.query(
      `
      SELECT id,name,email,phone,password_hash,status,created_at
      FROM sellers
      WHERE LOWER(email)=LOWER($1)
      LIMIT 1
      `,
  ...
  try {
    const result = await pool.query(`
      SELECT
        ...
        p.description,p.stock,p.discount_percent,
        p.created_at,p.seller_id,
        s.name AS seller_name
      FROM products p
      LEFT JOIN sellers s ON s.id=p.seller_id
      WHERE p.seller_id IS NULL OR s.status='active'
      ORDER BY p.id DESC
    `);
    ...
    res.json(result.rows.map(p => {
      ...
      return {
        ...
        sale_price: salePrice(price, discount),
        stock: integer(p.stock),
        seller_name: p.seller_name || "Shrivi"
      };
    }));
  } catch (error) {
...
async function getProductInput(body, isEdit = false) {
...
app.post("/api/products", requireAdmin, async (req, res) => {
  try {
    const p = await getProductInput(req.body);
    ...
    try {
      sellerId = await validateSellerId(req.body?.seller_id);
    } catch (error) {
      if (error.message === "INVALID_SELLER")
        return res.status(400).json({
    ...
    const result = await pool.query(
      ...
      (name,price,category,image,description,stock,discount_percent,seller_id)
...
app.put("/api/products/:id", requireAdmin, async (req, res) => {
  try {
    ...
    const p = await getProductInput(req.body, true);
...
app.post("/api/seller/products", requireSeller, async (req, res) => {
  try {
    ...
    console.error("Seller add product:", error);
    res.status(500).json({
      error: "Failed to add product"
    });
  }
...
app.put("/api/seller/products/:id", requireSeller, async (req, res) => {
  try {
    const id = integer(req.params.id);
    ...
    console.error("Seller edit product:", error);
    res.status(500).json({
      error: "Failed to update seller product"
    });
  }
});
...
app.delete("/api/seller/products/:id", requireSeller, async (req, res) => {
  try {
    ...
    res.status(500).json({
      error: "Failed to delete seller product"
    });
  }
});
...
app.post("/api/orders", async (req, res) => {
  ...
  try {
    const name = cleanText(req.body?.customer_name);
    ...
    for (const item of items) {
      ...
      quantityMap.set(id, totalQuantity);
    }
    ...
    for (const id of ids) {
      ...
      const finalPrice = salePrice(price, discount);
    ...
    for (const id of ids) {
      ...
      if (!updated.rows.length) {
        ...
        return res.status(409).json({
          error: "Stock changed. Please refresh your cart."
        });
      }
    }
  ...
  try {
    ...
    const result = await pool.query(
      ...
      WHERE EXISTS (
        ...
        WHERE NULLIF(item->>'seller_id','')::integer=$1
      )
      ORDER BY id DESC
      `,
      [sellerId]
    ...
    res.json(result.rows.map(order => {
      ...
      const sellerItems = allItems.filter(
        item => Number(item?.seller_id) === sellerId
      );
...
app.get("/api/seller/dashboard", requireSeller, async (req, res) => {
  try {
    ...
    res.json({
      products: integer(products.rows[0].count),
      stock: integer(stock.rows[0].stock),
      orders: integer(orders.rows[0].count),
...
app.put("/api/seller/orders/:id/status", requireSeller, async (req, res) => {
  try {
    const id = integer(req.params.id);
    const status = cleanText(req.body?.status).toLowerCase();
    ...
    if (!check.rows.length)
      return res.status(404).json({
        error: "Order not found for this seller"
      });
    ...
    console.error("Seller status:", error);
    res.status(500).json({
      error: "Failed to update order status"
    });
  }
...
app.get("/api/admin/sellers", requireAdmin, async (req, res) => {
  try {
    ...
    res.json(result.rows.map(seller => ({
      ...seller,
      product_count: integer(seller.product_count)
    })));
  } catch (error) {
...
app.put("/api/admin/sellers/:id/status", requireAdmin, async (req, res) => {
  try {
    const id = integer(req.params.id);
    const status = cleanText(req.body?.status).toLowerCase();
...
app.get("/api/dashboard", requireAdmin, async (req, res) => {
  try {
    ...
    res.json({
      products: integer(products.rows[0].count),
      orders: integer(orders.rows[0].count),
      sellers: integer(sellers.rows[0].count),
      customers: integer(customers.rows[0].count)
    });
  } catch (error) {
