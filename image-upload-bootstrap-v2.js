...
// =========================================================
// SHRIVI IMAGE UPLOAD BOOTSTRAP
// =========================================================
// server.js already contains the main application and Cloudinary
// configuration. This bootstrap adds the product-specific image
// endpoints and injects the upload UI into Admin/Seller pages.
// =========================================================
...
async function cloudinaryUpload(file, productId) {
  ...
  if (!cloud || !key || !secret) {
    throw Object.assign(
      new Error("Image storage is not configured"),
      { statusCode: 503 }
    );
  }
...
// =========================================================
// server.js uses res.sendFile() for / and /seller, so changing
// express.static alone cannot inject the UI. We therefore wrap
// app.get() before server.js is loaded and intercept only these
// two page routes.
// =========================================================
      ...
      return function (req, res, next) {
        ...
        res.sendFile = function (filePath, options, callback) {
          ...
          fs.readFile(filePath, "utf8", (error, html) => {
            ...
            const script =
              '<script src="/image-upload-ui.js?v=4"></script>';
...
function installRoutes(app) {
  if (app.__shriviImageRoutesV4) return;
  app.__shriviImageRoutesV4 = true;
  ...
  app.post(
    ...
    async (req, res) => {
      try {
        ...
        const id = positiveInt(req.params.id);
        const sellerId = positiveInt(
          req.session.seller.id
        );

        if (!id || !sellerId) {
          return res.status(400).json({
            error: "Invalid product or seller"
          });
        }
...
// Start the existing Shrivi application.
require("./server.js");
